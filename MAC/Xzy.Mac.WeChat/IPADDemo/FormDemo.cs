...
namespace MacDemo
{
    public partial class FormDemo : Form
    {
        ...
        {
            var res = weChatThread.Wx_MassMessage(txt_msgGroupWxid.Text, txt_msgGorupContext.Text);
            txt_syslog.Text = res.ConvertToString();
        }
        ...
        {
            var res = weChatThread.Wx_GetUserQRCode(txt_groupwxid.Text);
            txt_syslog.Text = res.ConvertToString();
        }
        ...
        {
            var res = weChatThread.Wx_GetRequestToken(txt_gzhid.Text, txt_gzhUrl.Text);
            txt_syslog.Text = res.ConvertToString();
        }
        ...
        {
            var res = weChatThread.Wx_SnsSync(txt_snskey.Text);
            txt_syslog.Text = res.ConvertToString();
        }
        ...
        {
            var res = JsonConvert.SerializeObject(weChatThread.userData);
            txt_syslog.Text = res.ConvertToString();
        }

        private void button54_Click(object sender, EventArgs e)
        {
            var res = weChatThread.Wx_ShareCard(txt_carduser.Text, txt_cardwxid.Text, txt_cardtitle.Text);
            txt_syslog.Text = res.ConvertToString();
        }

        private void button53_Click_1(object sender, EventArgs e)
        {
            WxTtsMsg msg = JsonConvert.DeserializeObject<WxTtsMsg>(txt_readpackBody.Text);
            var res = weChatThread.RedpackOK(txt_readpackBody.Text, msg.Timestamp);
        }

        private void button55_Click(object sender, EventArgs e)
        {
            var res = weChatThread.Wx_ESendLink(txt_snsLinkTitle.Text, txt_snsLinkDesc.Text, txt_snsLinkUrl.Text);
            txt_syslog.Text = res.ConvertToString();
        }

        private void button57_Click(object sender, EventArgs e)
        {
            var res = weChatThread.Wx_SendMomentVideo();
            txt_syslog.Text = res.ConvertToString();
        }

        private void btn_acceptMsg_Click(object sender, EventArgs e)
        {
            try
            {
                var res = weChatThread.Wx_AcceptUser(txt_v1.Text,txt_v2.Text);
                txt_syslog.Text = res.ConvertToString();
            }
            catch (Exception) { }
        }
